...
\begin{lstlisting}[language=Python,caption={Basic Test from \texttt{tutorial.ipynb}}]
...
This function then asserts that certain properties hold for each value of $x$ generated. In this example, it tests that the built-in absolute value function of Python works as intended.
...
\begin{lstlisting}[language=Python,caption={Complex Inputs from \texttt{tutorial.ipynb}}]
...
\vspace{5mm}
\noindent In order to improve test robustness, it can be useful to increase the number of tests generated. Furthermore, generating large amounts of random input helps to approximate formal verification. The number of test cases that are generated can be changed with the \texttt{settings} decorator and the \texttt{max\_samples} property:

\begin{lstlisting}[language=Python,caption={Specifying Test Case Amount from \texttt{tutorial.ipynb}}]
...
\begin{lstlisting}[language=Python,caption={Specifying Seeds from \texttt{tutorial.ipynb}}]
